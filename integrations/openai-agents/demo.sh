...
OUTPUT_ROOT="/tmp/stipul-openai-agents-demo"
TEST_NODES=(
  "integrations/openai-agents/test_openai_agents_stdio.py::test_allowed_safe_read_returns_stable_output_and_writes_evidence"
  "integrations/openai-agents/test_openai_agents_stdio.py::test_denied_and_kill_switch_paths_are_structured_and_logged"
)
...
printf 'transport=agents.mcp.MCPServerStdio -> stipul gateway mcp stdio\n'
printf 'driver=focused integration test nodes plus stipul verify\n'
printf '## Demo Execution\n'
printf '$ %s\n' "STIPUL_OPENAI_AGENTS_DEMO=1 STIPUL_OPENAI_AGENTS_OUTPUT_DIR=$OUTPUT_ROOT .venv/bin/python -m pytest -s -q ${TEST_NODES[*]}"
STIPUL_OPENAI_AGENTS_DEMO=1 \
STIPUL_OPENAI_AGENTS_OUTPUT_DIR="$OUTPUT_ROOT" \
...
printf '$ %s\n' ".venv/bin/python -c \"from pathlib import Path; import sys; sys.path.insert(0, '$REPO_ROOT/integrations/openai-agents'); from openai_agents_stdio import tamper_last_event; tamper_last_event(Path('$OUTPUT_ROOT/tampered-session/events.jsonl'))\""
.venv/bin/python -c "from pathlib import Path; import sys; sys.path.insert(0, '$REPO_ROOT/integrations/openai-agents'); from openai_agents_stdio import tamper_last_event; tamper_last_event(Path('$OUTPUT_ROOT/tampered-session/events.jsonl'))"
